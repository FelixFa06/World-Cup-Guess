...
SQLAlchemy ORM (models.py) → SQLite (instance/guess.db)
  ├── User / Match / Project1Pick / Project2Pick / MatchPrediction / DailyStar
```

## 核心算分规则 (scoring.py)

- **项目一**：冠军 5分 / 金靴 5分 / 金球 5分 — `score_project1()`
- **项目二**：4个分区各选1队进四强，每对1队 2分 — `score_project2()`
- **项目三**：比分全对 3分，胜负关系对 1分；总进球≥5 或 分差≥3 → 猜对比分 5分；决赛翻倍 — `score_match_prediction()`
...
- **截止控制**：项目一/二各有限时窗口，项目三每场开赛前截止，由 `_is_p1_deadline_passed()` 等函数判断。
