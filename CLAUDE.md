...
rm -f instance/guess.db && python scripts/init_db.py
python run.py &
...
src/config.py 中配置项前缀也已重命名：`P2_GROUP_PTS` 表示项目二小组赛分值，`P3_PER_TEAM_PTS` 表示项目三每队分值，`P4_*` 表示项目四规则。

## 核心算分规则 (src/scoring.py)
...
- **SQLite 要求 naive datetime**：项目中使用 `utcnow()` (src/__init__.py 中定义) 返回无时区的 UTC 时间。**禁止**使用 `datetime.now(timezone.utc)` 直接写入数据库。
...
- `scripts/init_db.py` 含迁移逻辑：自动为新旧数据库补充 `country_code`
...
`scripts/deploy.sh` 在 Ubuntu 22.04 上完成：系统更新 → 安装 Python/Nginx → 创建 venv → 初始化 DB → 配置 systemd + Gunicorn → 配置 Nginx 反向代理 → 开启防火墙。
