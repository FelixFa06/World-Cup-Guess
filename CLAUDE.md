...
- 取值：`"open"` 强制开放 / `"closed"` 强制截止 / `""` 自动（按比赛时间判断）
- API：`POST /api/admin/settings/<key>` 设置值
- `_is_pN_deadline_passed()` 函数优先检查 `_is_project_closed_by_admin()`
- `_is_p3_open()` 先检查管理员是否关闭，再检查是否手动开放
- 前端模板显示截止原因（管理员截止 vs 自动截止），用户明确知晓
