# ⚽ 世界杯竞猜平台

2026 世界杯球迷群竞猜系统 — 一站式提交预测、自动算分、实时排名。

## 功能概览

| 功能 | 说明 |
|------|------|
| 🔐 账号系统 | 昵称 + 密码注册登录，管理员单独账号 |
| 📜 规则展示 | 三个竞猜项目完整规则，比分预测示例 |
| 🏆 项目一 | 冠军球队、金靴得主、金球奖得主预测 |
| 🏟️ 项目二 | 淘汰赛四强预测（4个分区各选一队） |
| ⚽ 项目三 | 32场淘汰赛单场比分竞猜 |
| 🧮 自动算分 | 录入比分后自动比对、算分、更新排名 |
| 🌾 收米之星 | 每日得分最高者获荣誉称号 |
| 📊 实时排名 | 总分排名 + 各项目单项得分 |
| 🔍 预测公示 | 赛后公开所有群友预测，透明可见 |
| 📱 移动端 | 响应式设计，微信浏览器打开直接用 |

## 项目结构

```
world-cup-guess/
├── app.py                 # Flask 应用主入口（页面路由 + API + CLI命令）
├── config.py              # 配置文件（积分规则、管理员账号、赛事日期）
├── models.py              # 数据库模型（6个表，SQLAlchemy ORM）
├── scoring.py             # 算分引擎（纯函数，含大球加成和决赛翻倍逻辑）
├── init_db.py             # 一键初始化数据库 + 创建管理员账号
├── test_e2e.py            # 端到端测试脚本（覆盖注册→预测→算分→排名）
├── deploy.sh              # Ubuntu 22.04 一键部署脚本
├── requirements.txt       # Python 依赖
├── CLAUDE.md              # AI 辅助开发文档
├── static/
│   └── style.css          # 自定义样式（Pico.css 主题 + 响应式）
└── templates/
    ├── layout.html        # 公共导航框架
    ├── index.html         # 首页（赛况概览 + 收米之星）
    ├── login.html         # 登录 / 注册
    ├── rules.html         # 竞猜规则完整展示
    ├── predict.html       # 三个竞猜项目的提交入口
    ├── matches.html       # 所有淘汰赛 + 赛后预测公示
    ├── rankings.html      # 积分排名 + 收米之星历史
    └── admin.html         # 管理后台（添加比赛、录入比分、结算项目）
```

## 技术栈

| 层面 | 选择 |
|------|------|
| 后端框架 | Python Flask |
| 数据库 | SQLite + SQLAlchemy ORM |
| 用户认证 | Flask-Login + Session |
| 前端 | Jinja2 模板 + Pico.css + 原生 JS |
| 生产部署 | Gunicorn + Nginx + systemd |

## 竞猜规则

### 活动简介

2026 年世界杯期间，可以组建球友参与竞猜活动。活动分为 **3 个项目**，各项目累计积分排名，**前三名有奖**。

---

### 🏆 项目一：冠军 · 金靴 · 金球

竞猜冠军球队、金靴得主、金球奖得主。决赛结束后结算。

| 预测项 | 猜对得分 |
|--------|---------|
| 冠军球队 | 5 分 |
| 金靴得主 | 5 分 |
| 金球奖得主 | 5 分 |
| 全部猜对 | **15 分** |

> 📌 提交截止：第一场淘汰赛开赛前。截止前可修改。

---

### 🏟️ 项目二：四强

世界杯淘汰赛共 32 支球队，分为 **4 个 1/4 分区**，每区 8 支球队。

群友在每个分区中选择一支可能进入四强的球队，共预测 4 支。每猜对一支得 **2 分**，满分 **8 分**。

> 📌 开放时间：16强出炉后 → 截止：16进8开赛前

---

### ⚽ 项目三：单场淘汰赛

世界杯共有 **32 场淘汰赛**，每场开赛前竞猜该场比赛比分。

> ⚠️ 只考虑**常规时间**比分，不考虑加时和点球大战。

**基本积分：**

| 情况 | 得分 |
|------|------|
| 比分完全正确 | **3 分** |
| 比分不完全正确，但胜负关系对（胜/负/平）| **1 分** |
| 胜负关系错误 | 0 分 |

**🏅 决赛加成：** 决赛猜对比分得 **6 分**，胜负关系对得 **2 分**。

**💥 大球加成：** 当常规时间 **总进球数 ≥ 5** 或 **分差 ≥ 3** 时，猜对比分可得 **5 分**（决赛则为 10 分）。

**举例说明：**

> **半决赛：葡萄牙 vs 英格兰**
>
> 赛前预测：玩家A预测葡 1-1 英，玩家B预测葡 2-1 英，玩家C预测葡 8-1 英
>
> 实际结果：**葡萄牙 8-1 英格兰**（总进球 9 ≥ 5，分差 7 ≥ 3 → 激活大球加成）
>
> - 玩家A：**0 分**（预测平局，实际主队胜）
> - 玩家B：**1 分**（胜负关系对，比分不对）
> - 玩家C：**5 分**（比分全对 + 大球加成）

---

### 🌾 收米之星

以**比赛日**为单位，每天颁给前一天所有比赛结算后得分最高的群友。

- 多人并列则**共享称号**，纯荣誉，不加分
- 排名页展示每位群友累计获得次数（🌾 × N）
- 整个赛事结束后，获得次数最多者授予 **「收米界最长的河」🏆** 称号

---

### 积分与排名

淘汰赛开始后，每场赛后更新积分榜。三项目合计总分排序，前三名有奖。

## 本地开发

### 环境要求

- Python 3.10+
- pip

### 快速开始

```bash
# 1. 安装依赖
pip install -r requirements.txt

# 2. 初始化数据库
python init_db.py

# 3. 启动开发服务器
python -m flask --app app run --host 0.0.0.0 --port 5000

# 4. 打开浏览器访问
# http://127.0.0.1:5000
```

默认管理员：昵称 `admin`，密码 `admin123`（可在 `config.py` 修改）。

### 运行测试

```bash
# 先清空数据库并重启服务器
rm -f instance/guess.db
python init_db.py
python -m flask --app app run --host 0.0.0.0 --port 5000 &

# 运行测试
PYTHONIOENCODING=utf-8 python test_e2e.py
```

## 部署到云服务器

### 1. 购买服务器

推荐阿里云 ECS：1核 2GB，Ubuntu 22.04 LTS，1-3 Mbps 带宽。

买完后获得**公网 IP**。

### 2. 连接到服务器

```bash
ssh root@你的服务器IP
```

> 如果提示 `Permission denied (publickey)`，去阿里云控制台「重置实例密码」并重启，再用密码登录。

### 3. 一键部署

```bash
# 先安装 git 并拉取代码
apt update && apt install -y git
git clone https://github.com/你的仓库/world-cup-guess.git /opt/world-cup-guess

# 执行部署脚本
cd /opt/world-cup-guess
sudo bash deploy.sh
```

`deploy.sh` 自动完成：
- 安装 Python、Nginx、Gunicorn
- 创建虚拟环境并安装依赖
- 初始化数据库
- 配置 systemd 服务（开机自启）
- 配置 Nginx 反向代理
- 开放防火墙 80 端口

### 4. 验收

浏览器访问 `http://你的服务器IP`，能看到首页即为成功。

### 5. 管理员密码

部署脚本会自动生成随机管理员密码。查看方式：

```bash
grep ADMIN_PASSWORD /etc/systemd/system/world-cup-guess.service
```

### 费用参考

| 项目 | 费用 |
|------|------|
| 云服务器（1核2G）| 约 100-300 元/年（新用户）|
| 带宽流量 | 约 5-15 元/月 |
| 域名（可选）| 约 5-50 元/年 |
| **一个月总计** | **不到 100 元** |

> 如果只用世界杯期间（一个月），买按量付费，用完释放即可。

## 管理后台操作

管理员登录后可在 `http://你的IP/admin` 执行：

| 操作 | 说明 |
|------|------|
| ➕ 添加比赛 | 逐个添加 32 场淘汰赛（轮次、对阵、时间）|
| 🔓 开放/关闭预测 | 控制每场比赛是否接受竞猜 |
| 📝 录入比赛结果 | 输入真实比分 → 自动算分 → 自动判定收米之星 |
| 🏆 结算项目一 | 输入冠军/金靴/金球 → 批量算分 |
| 🏟️ 结算项目二 | 输入四强球队 → 批量算分 |

## 配置文件说明

`config.py` 中可修改：

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `SECRET_KEY` | 随机字符串 | 生产环境务必修改 |
| `ADMIN_NICKNAME` | admin | 管理员昵称 |
| `ADMIN_PASSWORD` | admin123 | 管理员密码 |
| `P1_CHAMPION_PTS` | 5 | 项目一冠军分值 |
| `P1_GOLDEN_BOOT_PTS` | 5 | 项目一金靴分值 |
| `P1_GOLDEN_BALL_PTS` | 5 | 项目一金球分值 |
| `P2_PER_TEAM_PTS` | 2 | 项目二每队分值 |
| `P3_EXACT_PTS` | 3 | 项目三比分全对分值 |
| `P3_RESULT_PTS` | 1 | 项目三胜负关系对分值 |
| `P3_FINAL_MULTIPLIER` | 2 | 决赛翻倍倍数 |
| `P3_BIG_MATCH_GOALS` | 5 | 大球加成：总进球阈值 |
| `P3_BIG_MATCH_DIFF` | 3 | 大球加成：分差阈值 |
| `P3_BIG_MATCH_PTS` | 5 | 大球加成：猜对比分分值 |

## License

MIT
